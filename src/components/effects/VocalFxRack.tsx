...
export function VocalFxRack() {
  ...
  useEffect(() => {
    ...
    // Draw Response Curve
    ctx.beginPath();
    ctx.strokeStyle = fxEnabled ? '#a855f7' : 'rgba(255,255,255,0.15)';
    ctx.lineWidth = 2;
    ctx.shadowBlur = fxEnabled ? 8 : 0;
    ...
    dots.forEach(dot => {
      ...
      ctx.globalAlpha = fxEnabled ? 0.25 : 0.05;
      ...
      ctx.beginPath();
      ctx.globalAlpha = 1.0;
      ctx.fillStyle = fxEnabled ? '#ffffff' : '#3f3f46';
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (!activeDragRef.current || !fxEnabled) return;
    const canvas = eqCanvasRef.current;
    if (!canvas) return;
    const rect = canvas.getBoundingClientRect();
    const y = e.clientY - rect.top;
    ...
    <section className="bg-surface rounded-2xl p-4 flex flex-col gap-4 shadow-lg shadow-black/35 border border-white/[0.02]">
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 transition-all duration-300 ${fxEnabled ? 'opacity-100' : 'opacity-35 pointer-events-none'}`}>
        ...
        <div className="bg-black/20 rounded-xl p-3 border border-white/[0.02] flex flex-col gap-3">
          <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider flex items-center gap-1.5 font-sans border-b border-white/[0.02] pb-1">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="border-t border-white/[0.02] pt-2 flex flex-col gap-2">
              <label className="flex items-center gap-2 cursor-pointer select-none text-[9px] font-bold text-zinc-400 uppercase tracking-wider">
                <input 
                  type="checkbox" 
                  checked={fxSettings.doubleEnabled} 
                  onChange={(e) => setFxSettings({ doubleEnabled: e.target.checked })}
                  className="accent-primary w-3 h-3 rounded border-white/10 cursor-pointer"
                />
                <span>Stereo Doubler</span>
              </label>
              {fxSettings.doubleEnabled && (
                <input 
                  type="range" 
                  min="0" max="100" 
        ...
        <div className="bg-black/20 rounded-xl p-3 border border-white/[0.02] flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-3 justify-center h-full pb-1">
            <div>
        ...
        <div className="bg-black/20 rounded-xl p-3 border border-white/[0.02] flex flex-col gap-3 lg:col-span-2">
          <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider flex items-center justify-between border-b border-white/[0.02] pb-1 font-sans">
          ...
          <div className="h-28 relative rounded-lg border border-white/[0.03] overflow-hidden">
            <canvas 
              ref={eqCanvasRef} 
